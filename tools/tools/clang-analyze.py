...
def generate(env):
    assert(txconfig.CROSS_TOOL == 'clang-analyze')
    ...
    if txconfig.EXEC_PATH:
        if not os.path.exists(txconfig.EXEC_PATH):
            print
            print 'warning: txconfig.EXEC_PATH(%s) does not exists.' % txconfig.EXEC_PATH
            print
            return
        env.AppendENVPath('PATH', txconfig.EXEC_PATH)
