...
def GenCconfigFile(env, BuildOptions):
    ...
    if txconfig.PLATFORM == 'gcc':
        ...
        if not os.path.isfile('cconfig.h'):
            import gcc
            gcc.GenerateGCCConfig(txconfig)
...
def PrepareBuilding(env, root_directory, has_libcpu=False, remove_components = []):
    ...
    AddOption('--genconfig',
                ...
                action = 'store_true',
                default = False,
                help = 'Generate .config from txconfig.h')
    AddOption('--useconfig',
                dest = 'useconfig',
                type = 'string',
                help = 'make txconfig.h from config file.')
    AddOption('--verbose',
                dest = 'verbose',
                action = 'store_true',
                default = False,
    ...
    Env = env
    Tx_Root = os.path.abspath(root_directory)
    ...
    if tgt_name:
        ...
        SetOption('no_exec', 1)
        try:
            txconfig.CROSS_TOOL, txconfig.PLATFORM = tgt_dict[tgt_name]
            # replace the 'RTT_CC' to 'CROSS_TOOL'
            os.environ['RTT_CC'] = txconfig.CROSS_TOOL
            utils.ReloadModule(txconfig)
        except KeyError:
            print ('Unknow target: '+ tgt_name+'. Avaible targets: ' +', '.join(tgt_dict.keys()))
            sys.exit(1)
    elif (GetDepend('RT_USING_NEWLIB') == False and GetDepend('RT_USING_NOLIBC') == False) \
        and txconfig.PLATFORM == 'gcc':
        AddDepend('RT_USING_MINILIBC')

    # auto change the 'RTT_EXEC_PATH' when 'txconfig.EXEC_PATH' get failed
    if not os.path.exists(txconfig.EXEC_PATH):
        if 'RTT_EXEC_PATH' in os.environ:
            # del the 'RTT_EXEC_PATH' and using the 'EXEC_PATH' setting on txconfig.py
            del os.environ['RTT_EXEC_PATH']
            utils.ReloadModule(txconfig)

    # add compability with Keil MDK 4.6 which changes the directory of armcc.exe
    if txconfig.PLATFORM == 'armcc':
        if not os.path.isfile(os.path.join(txconfig.EXEC_PATH, 'armcc.exe')):
            if txconfig.EXEC_PATH.find('bin40') > 0:
                txconfig.EXEC_PATH = txconfig.EXEC_PATH.replace('bin40', 'armcc/bin')
    ...
    if env['PLATFORM'] == 'win32':
        os.environ['PATH'] = txconfig.EXEC_PATH + ";" + os.environ['PATH']
    else:
        os.environ['PATH'] = txconfig.EXEC_PATH + ":" + os.environ['PATH']

    # add program path
    env.PrependENVPath('PATH', txconfig.EXEC_PATH)
    # add txconfig.h/BSP path into Kernel group
    ...
    # parse txconfig.h to get used component
    ...
    if configfn:
        from menuconfig import mk_txconfig
        mk_txconfig(configfn)
        exit(0)
    ...
    # include kernel
    objs.extend(SConscript(Tx_Root + '/threadx/SConscript', variant_dir=kernel_vdir + '/src', duplicate=0))
    # include libcpu
    if not has_libcpu:
        objs.extend(SConscript(Tx_Root + '/threadx/ports/SConscript',
                    variant_dir=kernel_vdir + '/libcpu', duplicate=0))

    # include components
    # objs.extend(SConscript(Tx_Root + '/components/SConscript',
...
def PrepareModuleBuilding(env, root_directory, bsp_directory):
    ...
    Env = env
    Tx_Root = root_directory

    # parse bsp txconfig.h to get used component
    PreProcessor = PatchedPreProcessor()
    f = open(bsp_directory + '/txconfig.h', 'r')
    contents = f.read()
    f.close()
    PreProcessor.process_contents(contents)
    BuildOptions = PreProcessor.cpp_namespace
...
def GenTargetProject(program = None):
    ...
    if GetOption('target') == 'ua':
        ...
        PrepareUA(Projects, Tx_Root, str(Dir('#')))
...
def EndBuilding(target, program = None):
    ...
    if hasattr(txconfig, 'BSP_LIBRARY_TYPE'):
        Env['bsp_lib_type'] = txconfig.BSP_LIBRARY_TYPE

    if hasattr(txconfig, 'dist_handle'):
        Env['dist_handle'] = txconfig.dist_handle

    Env.AddPostAction(target, txconfig.POST_ACTION)
    ...
    if GetOption('make-dist') and program != None:
        ...
        MkDist(program, BSP_ROOT, Tx_Root, Env)
    if GetOption('make-dist-strip') and program != None:
        from mkdist import MkDist_Strip
        MkDist_Strip(program, BSP_ROOT, Tx_Root, Env)
        need_exit = True
    if GetOption('cscope'):
    ...
    if not GetOption('help') and not GetOption('target'):
        if not os.path.exists(txconfig.EXEC_PATH):
            print ("Error: the toolchain path (" + txconfig.EXEC_PATH + ") is not exist, please check 'EXEC_PATH' in path or txconfig.py.")
            need_exit = True
...
def GetVersion():
    ...
    rtdef = os.path.join(Tx_Root, 'include', 'rtdef.h')
