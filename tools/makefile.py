...
def TargetMakefile(env):
    ...
    cross = os.path.abspath(txconfig.EXEC_PATH)
    cross = os.path.join(cross, txconfig.PREFIX)
    make.write('CROSS_COMPILE ?=%s' % cross.replace('\\', '\\\\'))
    make.write('\n')
    make.write('\n')

    make.write('CFLAGS :=%s' % (txconfig.CFLAGS))
    make.write('\n')
    make.write('AFLAGS :=%s' % (txconfig.AFLAGS))
    make.write('\n')
    make.write('LFLAGS :=%s' % (txconfig.LFLAGS))
    make.write('\n')
    if 'CXXFLAGS' in dir(txconfig):
        make.write('CXXFLAGS :=%s' % (txconfig.CXXFLAGS))
        make.write('\n')
