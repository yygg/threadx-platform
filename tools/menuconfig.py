...
# make txconfig.h from .config

def mk_txconfig(filename):
    try:
    ...
    txconfig = open('txconfig.h', 'w')
    txconfig.write('#ifndef RT_CONFIG_H__\n')
    txconfig.write('#define RT_CONFIG_H__\n\n')
    ...
    for line in config:
        ...
        if line[0] == '#':
            ...
            if len(setting) >= 2:
                ...
                if setting[1] == 'y':
                    txconfig.write('#define %s\n' % setting[0])
                else:
                    txconfig.write('#define %s %s\n' % (setting[0], re.findall(r"^.*?=(.*)$",line)[0]))

    if os.path.isfile('txconfig_project.h'):
        txconfig.write('#include "txconfig_project.h"\n')

    txconfig.write('\n')
    txconfig.write('#endif\n')
    txconfig.close()
...
def pyconfig_silent(RTT_ROOT):
    ...
    # silent mode, force to make txconfig.h
    mk_txconfig(fn)
