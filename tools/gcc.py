...
def GetGCCRoot(txconfig):
    exec_path = txconfig.EXEC_PATH
    prefix = txconfig.PREFIX
...
def CheckHeader(txconfig, filename):
    root = GetGCCRoot(txconfig)
...
def GetNewLibVersion(txconfig):
    version = 'unknown'
    root = GetGCCRoot(txconfig)

    if CheckHeader(txconfig, '_newlib_version.h'): # get version from _newlib_version.h file
        ...
        if f:
            ...
            f.close()
    elif CheckHeader(txconfig, 'newlib.h'): # get version from newlib.h
...
def GCCResult(txconfig, str):
    ...
    gcc_cmd = os.path.join(txconfig.EXEC_PATH, txconfig.CC)
...
def GenerateGCCConfig(txconfig):
    str = ''
    cc_header = ''
    ...
    cc_header += '\n'
    cc_header += GCCResult(txconfig, str)
    cc_header += '\n#endif\n'
